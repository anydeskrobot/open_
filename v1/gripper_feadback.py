...
GRIPPER_CHECK_DELAY = 35  # Seconds to wait before checking gripper status
    ...
    def listener_callback(self, msg):
        if len(msg.data) >= 2:
            ...
            if effort <= -0.6110:
                ...
                self.get_logger().info("Object successfully picked")
            #if position <= -0.0200 and effort <= -0.6110:
               # self.object_picked = True
               # self.get_logger().info("Object successfully picked")
            else:
                self.get_logger().info("Object not picked yet")
...
class ArucoDetector(Node):
    def __init__(self):
        ...
        self.REQUIRED_CONSISTENT_COUNT = 5
    ...
    def check_gripper_status(self):
        """Check if the object was picked within the timeout period"""
        gripper_listener = GripperListener()
        
        # Wait for GRIPPER_CHECK_DELAY seconds while processing messages
        start_time = time.time()
        while time.time() - start_time < GRIPPER_CHECK_DELAY and rclpy.ok():
            rclpy.spin_once(gripper_listener, timeout_sec=0.1)
        
        return gripper_listener.object_picked
    ...
    def save_to_json_and_exit(self, position_data):
        ...
        if process:
            ...
            if not self.check_gripper_status():
                print("Object not picked after timeout - restarting process")
